...
let scrapGoogleScholarPageCodeToRunOnChromium = () => {
  ...
  var extractJournalFromGoogleScholarAuthorAndYearDiv = function(text) {
    ...
    if (splittedTextList.length > 1) return splittedTextList[1];
    else if (splittedTextList.length > 0) return splittedTextList[0];
    else return "";
  };
  ...
  for (var element of elements) {
    ...
    if (authorAndDivSearchResult.length > 0) {
      ...
      authors = extractAuthorsFromGoogleScholarAuthorAndYearDiv(
        authorAndYearText
      );
      journal = extractJournalFromGoogleScholarAuthorAndYearDiv(
        authorAndYearText
      );
    }
...
function searchGoogleScholar(searchText, callback) {
  searchGoogleAndScrapResults(searchText)
    .then(resultObjectList => {
...
function getCitedbyOfArticle(citedByLink, callback) {
  scrapCitedByPage("http://scholar.google.com" + citedByLink)
    .then(resultObjectList => {
      callback(null, resultObjectList);
    })
    .catch(function(err) {
