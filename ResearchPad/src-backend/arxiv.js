...
function searchQueryCreator(query) {
  ...
  for (let i = 0; i < newQuery.length; i++) {
    if (i < newQuery.length - 1) new_Query += newQuery[i] + "+AND+";
    else new_Query += newQuery[i];
  }
  return { q: new_Query };
}
...
makeUrl = function(query, max_results, sort_by) {
  ...
  console.log(
    "http://export.arxiv.org/api/query?sortBy=" +
      sort_by +
      "&max_results=" +
      max_results +
      "&search_query=" +
...
async function search(query, cb) {
  ...
  return request.get(makeUrl(coerceQuery(search_query)), function(
    err,
    resp,
    data
  ) {
    return xml2js.parseString(data, function(err, parsed) {
      ...
      if (err != null) {
        ...
        items =
          parsed != null
            ? (ref = parsed.feed) != null
              ? (ref1 = ref.entry) != null
                ? ref1.map(coerceEntry)
                : void 0
              : void 0
            : void 0;
        items || (items = []);
