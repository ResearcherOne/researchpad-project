function SiblingConnection(ID, firstNodeID, secondNodeID) {
	...
	this.visualObj = visualizerModule.connectVisualObjectsByID(
		this.firstNodeID,
		this.secondNodeID
	);
...
function KnowledgeTree(
	...
	var reconstructRootNodes = function(serializedRootNodes) {
		...
		for (var serializedRootNodeID in serializedRootNodes) {
			...
			reconstructedRootNodes[serializedRootNodeID] = new RootNode(
				rootNodeData.ID,
				rootNodeData.academicDataLibrary,
				rootNodeData.radius,
				rootNodeData.x,
				rootNodeData.y,
				nodeDragStartCallback,
				nodeDragEndCallback,
				nodeMouseOverCallback,
				nodeMouseOutCallback,
				nodeClickedCallback
			);
			reconstructedRootNodes[serializedRootNodeID].importSerializedReferences(
				rootNodeData.references,
				rootNodeData.citedByNodes,
				rootNodeData.referenceCount,
				rootNodeData.citedByCount,
				rootNodeData.suggestedCitedByNodeList,
				rootNodeData.suggestedReferenceNodeList
			);

			reconstructedRootNodes[serializedRootNodeID].siblingIDs =
				rootNodeData.siblingIDs;
			reconstructedRootNodes[serializedRootNodeID].siblingCount =
				rootNodeData.siblingCount;

			const citedByNodes = reconstructedRootNodes[
				serializedRootNodeID
			].getAllCitedByNodes();
			this.citedByNodes = { ...this.citedByNodes, ...citedByNodes };

			const referenceNodes = reconstructedRootNodes[
				serializedRootNodeID
	...
	var reconstructSiblingConnections = function(serializedSiblingConnections) {
		...
		for (var siblingConnectionID in serializedSiblingConnections) {
			var siblingConnectionData = JSON.parse(
				serializedSiblingConnections[siblingConnectionID]
			);
			reconstructedSiblingConnections[
				siblingConnectionID
			] = new SiblingConnection(
				siblingConnectionData.ID,
				siblingConnectionData.firstNodeID,
				siblingConnectionData.secondNodeID
			);
		}
	...
	var setSiblingReference = function(
		self,
		rootID,
		siblingReferenceRootID,
		firstNodeType,
		secondNodeType
	) {
	...
	var connectRootNodeToExistingLeafNodes = function(
		self,
		rootNodeID,
		leafNodeID,
		radius,
	...
	var suggestCitedBy = function(self, rootID, refID, suggestionPosition) {
		...
		self.citedByNodes[refID].setPositionForSuggestion(
			rootID,
			suggestionPosition
		);
	};
	...
	var suggestReference = function(self, rootID, refID, suggestionPosition) {
		...
		self.referenceNodes[refID].setPositionForSuggestion(
			rootID,
			suggestionPosition
		);
	};
	...
	this.createRootNode = function(ID, initialAcademicDataLibrary, radius, x, y) {
		if (this.citedByNodes[ID]) {
			...
			this.rootNodes[ID] = new RootNode(
				ID,
				initialAcademicDataLibrary,
				radius,
				x,
				y,
				nodeDragStartCallback,
			...
			const rootNodesIdListOfLeafNode = this.referenceNodes[
				ID
			].getRootNodeIdList();
			for (var i = 0; i < rootNodesIdListOfLeafNode.length; i++) {
	...
	this.addReferenceToRootNode = function(
		rootID,
		refID,
		initialAcademicDataLibrary,
		radius
		...
		if (!this.isRootNodeExists(refID)) {
			if (isLeafNodeExists(this, refID)) {
				connectRootNodeToExistingLeafNodes(
					this,
					rootID,
					refID,
					radius,
				...
				const suggestedReferenceCount = this.rootNodes[
					rootID
				].getSuggestedReferenceCount();
	...
	this.removeReferenceFromRootNode = function(rootID, refID) {
		...
		if (isSuggested) {
			...
			this.hideLeafNodes(
				rootID,
				DEFAULT_ROOT_UPDATE_LEAF_NODE_REVEAL_DURATION_SEC
			);
		}
	...
	this.addCitedbyToRootNode = function(
		rootID,
		refID,
		initialAcademicDataLibrary,
		radius
		...
		if (!this.isRootNodeExists(refID)) {
			if (isLeafNodeExists(this, refID)) {
				...
				const suggestedCitedByCount = this.rootNodes[
					rootID
				].getSuggestedCitedByCount();
	...
	this.getCameraPosition = function() {
		return {
			x: visualizerModule.getCanvasPos().x,
			y: visualizerModule.getCanvasPos().y
		};
	};
	...
	this.serialize = function() {
		...
		serializedKnowledgeTree.siblingConnections = mapSerializeCallOnObj(
			this.siblingConnections
		);
		serializedKnowledgeTree.siblingConnectionCount = this.siblingConnectionCount;

		serializedKnowledgeTree.citedByNodes = mapSerializeCallOnObj(
			this.citedByNodes
		);
		serializedKnowledgeTree.referenceNodes = mapSerializeCallOnObj(
			this.referenceNodes
		);
...
*/
	...
	this.hideLeafNodes = function(rootNodeID, hideDurationSec) {
		...
		this.rootNodeApperanceTimers[rootNodeID] = setTimeout(
			function(self) {
				for (var i = 0; i < citedByList.length; i++) {
					const ID = citedByList[i];
	...
	this.transformCitationNodeToRootNode = function(
		...
		const academicDataLibrary = this.citedByNodes[
			nodeIdToTransform
		].getAcademicDataLibrary();

		const rootNodesIdListOfLeafNode = this.citedByNodes[
			nodeIdToTransform
		].getRootNodeIdList();
		for (var i = 0; i < rootNodesIdListOfLeafNode.length; i++) {
			this.removeCitedbyFromRootNode(
				rootNodesIdListOfLeafNode[i],
				nodeIdToTransform
			);
		}
	...
	this.transformReferenceNodeToRootNode = function(
		rootNodeIdOfLeafNode,
		nodeIdToTransform,
		newNodeRadius,
		x,
		...
		const academicDataLibrary = this.referenceNodes[
			nodeIdToTransform
		].getAcademicDataLibrary();
		...
		for (var i = 0; i < rootNodesIdListOfLeafNode.length; i++) {
			this.removeReferenceFromRootNode(
				rootNodesIdListOfLeafNode[i],
				nodeIdToTransform
			);
		}
