...
function createRootNodeFromGoogleScholar(
	...
	var rootNodeObjectID = knowledgeTree.createRootNode(
		metadata,
		rootNodeRadius,
		x,
		y
...
function searchBarElementDragFinished(
	...
	if (isTargetPointsKnowledgeTree) {
		...
		var fullMetadata = lastAcademicSearchDataList[
			elementPositionInSearchData
		].getFullMetadata();

		createNewRootNodeOnKnowledgeTree(
...
function citationNodeDragEnd(nodeObj) {
	...
	if (citedByLink) {
		...
		var newRootNodeID = knowledgeTree.createRootNode(
			citedByNodeMetadata,
			rootNodeRadius,
			x,
			y
