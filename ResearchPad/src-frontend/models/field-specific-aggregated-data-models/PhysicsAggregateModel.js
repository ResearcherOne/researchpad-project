function PhysicsAggregateModel(arxivMetadata, semanticScholarMetadata) {
  ...
  this.getNodeFullContentData = function() {
    const title =
      this.arxivData.getTitle() ||
      this.semanticScholarData.getTitle() ||
      "No title";
    const year =
      this.arxivData.getYear() || this.semanticScholarData.getYear() || "?";
    const citationCount = this.semanticScholarData.getCitationCount() || "?";
    const abstract =
      this.arxivData.getAbstract() || "Abstract is not available.";
    const authors =
      this.arxivData.getAuthorList() ||
      this.semanticScholarData.getAuthorList();
